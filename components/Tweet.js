...
const Tweet = (props) => {
  ...
  const downloadScreenshot = () => {
    ...
    .then(function (dataUrl) { //download functionality
    ...
    <WrapperSection>
      ...
      <TweetBox> {/* container for whole tweet */}
        ...
        <ProfileInfoBox> {/* container for profile info */}
          ...
          <UserIdentityBox> {/* container for name & username */}
            <NameContainer> {/* container for name & checkmark SVG & settings SVG */}
              ...
              <SettingsSVG>
                <svg xmlnsXlink="http://www.w3.org/1999/xlink" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" className="r-4qtqp9 r-yyyyoo r-ip8ujx r-dnmrzs r-1p4rafz r-bnwqim r-1plcrui r-lrvibr"><g><circle cx="5" cy="12" r="2" /><circle cx="12" cy="12" r="2" /><circle cx="19" cy="12" r="2" /></g></svg>
              </SettingsSVG>
            </NameContainer>
        ...
        <ContentBox> {/* container for tweet AND engagement metrics */}
          <Content>
            <ReactCSSTransitionReplace transitionName="switch" transitionEnterTimeout={300} transitionLeaveTimeout={300}>
              {tweetGenerated ? <span key={1}>{tweetText}</span> : <TweetInput key={2} autoFocus={true} placeholder="type your tweet here" rows="4" value={tweetText} onChange={event => updateText(event.target.value)} />}
            </ReactCSSTransitionReplace>
          </Content>
          <DateTimeBox>
            {timeSelectorVisible ? <DatetimeInput key={3} type="datetime-local" min="2006-03-21T21:50" max={maxTime} value={datetimeSaved} onChange={event => updateDatetime(event.target.value)} /> : <span key={4} onClick={() => handleEditClick(toggleTimeSelector)}><span>{timeSaved}</span><span> · </span><span>{dateSaved}</span></span> }
            <span> · </span>
          ...
          <Engagement>
            <EngagementBox onClick={() => handleEditClick(toggleRTSelector)}> {/* RT */}
              {rtSelectorVisible ? <EngagementInput autoFocus={true} type="text" maxLength="6" value={rtAmount} onChange={(event) => setRT(event.target.value)} /> : <FatNumber>{rtAmount}</FatNumber>}
              <span> Retweets</span>
            </EngagementBox>
            <EngagementBox onClick={() => handleEditClick(toggleQTSelector)}> {/* QT */}
              {qtSelectorVisible ? <EngagementInput autoFocus={true} type="text" maxLength="6" value={qtAmount} onChange={(event) => setQT(event.target.value)} /> : <FatNumber>{qtAmount}</FatNumber>}
              <span style={{"white-space": "nowrap"}}> Quote Tweets</span>
            </EngagementBox>
            <EngagementBox onClick={() => handleEditClick(toggleLikeSelector)} style={{"marginRight":"0px"}}> {/* Likes */}
              {likeSelectorVisible ? <EngagementInput autoFocus={true} type="text" maxLength="6" value={likeAmount} onChange={(event) => setLikes(event.target.value)} /> : <FatNumber>{likeAmount}</FatNumber>}
              <span> Likes</span>
            </EngagementBox>
          </Engagement>
        </ContentBox>
      ...
      { !tweetGenerated ? <ButtonContainer><TwitterButton onClick={() => props.toggleThemeFunc()} style={{ "display": "flex", "align-items": "center", "justify-content": "center" }}><svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="rgb(255,255,255)" width="18px" height="18px"><g><path d="M0,0h24v24H0V0z" fill="none" /></g><g><g><g><path d="M20,15.31l1.9-1.9c0.78-0.78,0.78-2.05,0-2.83L20,8.69V6c0-1.1-0.9-2-2-2h-2.69l-1.9-1.9c-0.78-0.78-2.05-0.78-2.83,0 L8.69,4H6C4.9,4,4,4.9,4,6v2.69l-1.9,1.9c-0.78,0.78-0.78,2.05,0,2.83l1.9,1.9V18c0,1.1,0.9,2,2,2h2.69l1.9,1.9 c0.78,0.78,2.05,0.78,2.83,0l1.9-1.9H18c1.1,0,2-0.9,2-2V15.31z M12,18V6c3.31,0.01,6,2.69,6,6S15.31,18,12,18z" /></g></g></g></svg></TwitterButton><TwitterButton style={{ "width": "150px", "opacity": `${generateButtonOpacity}`, "transition": "all 0.2s" }} onClick={() => saveButtonClick()}>Generate Tweet</TwitterButton></ButtonContainer> : <ButtonContainer><TwitterButton onClick={() => generateTweet(false)} style={{ "display": "flex", "align-items": "center", "justify-content": "center" }}><svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="rgb(255,255,255)" width="18px" height="18px"><path d="M22.7 19l-9.1-9.1c.9-2.3.4-5-1.5-6.9c-2-2-5-2.4-7.4-1.3L9 6L6 9L1.6 4.7C.4 7.1.9 10.1 2.9 12.1c1.9 1.9 4.6 2.4 6.9 1.5l9.1 9.1c.4.4 1 .4 1.4 0l2.3-2.3c.5-.4.5-1.1.1-1.4z" fill="#ffffff"/></svg></TwitterButton><TwitterButton onClick={() => downloadScreenshot()} style={{ "width": "150px", "transition": "all 0.2s" }} >Download as JPG</TwitterButton></ButtonContainer>}
    </WrapperSection>
  );
}
